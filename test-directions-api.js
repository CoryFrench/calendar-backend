...
async function testDirectionsAPI() {
  try {
    ...
    const params = {
      origin: '825 Parkway St Suite 8, Jupiter, FL 33477',
      destination: '4570 SE Federal Hwy, Stuart, FL',
      mode: 'driving',
      departure_time: 'now', // Include for traffic info
      key: process.env.ROUTES_API_KEY
    };

    console.log('Making test request to Directions API...');
    ...
    if (response.data.status === 'OK') {
      console.log('✅ API key is working correctly with Directions API!');
      ...
      if (leg.duration_in_traffic) {
        console.log(`- Duration with traffic: ${leg.duration_in_traffic.text}`);
      } else {
      ...
      console.log('1. Check if the API key has the Directions API enabled in Google Cloud Console');
